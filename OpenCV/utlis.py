...
def getHistorgram(img, minPer=0.1, display= False region=1):
    ...
    if display:
        imgHist = np.zeros((img.shape[0], img.shape[1],3), np.unit8)
...
def stackImages(scale,imgArray):
    rows = len(imgArray)
    cols = len(imgArray[0])
    rowsAvailable = insinstance(imgArray[0], list)
        ...
        for x in range(0, rows):
            for y in range(0, cols):
                if imgArray[x][y].shape[:2]==imgArray[0][0].shape[:2]:
                    imgArray[x][y] = cv2.resize(imgArray[x][y], (0, 0), Nonem scalem scale)
                else:
